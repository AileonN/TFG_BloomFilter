...
/* BLOOM FILTER */
uint64_t BF_TACCESS = 0, BF_THIT = 0, BF_TMISS = 0;
...
void print_roi_stats(uint32_t cpu, CACHE* cache, uint32_t cacheNumber)
{
  ...
  if (cacheNumber == 7) { //LLC VALUES: load, rfo, prefetch
    BF_TACCESS = cache->roi_access[cpu][0] + cache->roi_access[cpu][1] + cache->roi_access[cpu][2];
    BF_THIT = cache->roi_hit[cpu][0] + cache->roi_hit[cpu][1] + cache->roi_hit[cpu][2];
    BF_TMISS = cache->roi_miss[cpu][0] + cache->roi_miss[cpu][1] + cache->roi_miss[cpu][2]; 
  }
  if (TOTAL_ACCESS > 0) {
    cout << cache->NAME;
...
int main(int argc, char** argv)
{
  ...
  */
  ...
  uint32_t npipe = 0;
  string bloom_type = "B";
  uint32_t BrainAddress = 0;
  
  int c;
  while ((c = getopt_long_only(argc, argv, "w:i:hc:b:p:s:m:n:f", long_options, NULL)) != -1 && !traces_encountered) {
    switch (c) {
    case 'w':
  ...
  for (auto it = caches.rbegin(); it != caches.rend(); ++it)
    (*it)->impl_replacement_final_stats(nHash, P, length, MAX, bloom_type,npipe,BF_TACCESS,BF_THIT,BF_TMISS,ooo_cpu[0]->finish_sim_cycle);
