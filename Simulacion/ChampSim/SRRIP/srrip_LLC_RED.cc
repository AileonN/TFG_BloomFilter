                                     ...
                                     uint8_t hit)
{
  ...
  if (Struct && !hit && NUM_WAY == LLC_WAYS && NUM_SET == LLC_SETS && (type == LOAD || type == RFO || type == PREFETCH)){
      ...
      PCentry = (ip >> 2) & (PCs-1);
      bool resBrain = b->Find(block_address);
 
      present = lookup(cpu, ip, block_address);
	  if (fullFilled){ 
        //Update stats when cache is full
		if (present){
	      if (resBrain){ TP++;} else { FP++;}
		}else{
          if (resBrain){ FN++;} else {TN++;}
		 }
       }
...
void CACHE::replacement_final_stats(uint16_t nHash, float P, uint32_t length, uint32_t MAX, string bloom_type, uint32_t npipe,uint64_t TOTAL_ACCESS, uint64_t TOTAL_HIT, uint64_t TOTAL_MISS, uint64_t cycles) {
  ...
  if (NUM_SET==LLC_SETS && NUM_WAY==LLC_WAYS){
	  write(npipe,&TP,sizeof(uint64_t));write(npipe,&FP,sizeof(uint64_t));
    write(npipe,&FN,sizeof(uint64_t));write(npipe,&TN,sizeof(uint64_t));
