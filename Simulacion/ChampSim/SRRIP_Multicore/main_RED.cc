...
void print_roi_stats(uint32_t cpu, CACHE* cache, uint32_t cacheNumber)
{
  ...
  if (cacheNumber == 100 || cacheNumber==75 || cacheNumber== 50 || cacheNumber==25){
    BF_TACCESS[cpu] = cache->roi_access[cpu][0] + cache->roi_access[cpu][1] + cache->roi_access[cpu][2];
    BF_THIT[cpu] = cache->roi_hit[cpu][0] + cache->roi_hit[cpu][1] + cache->roi_hit[cpu][2];
    BF_TMISS[cpu] = cache->roi_miss[cpu][0] + cache->roi_miss[cpu][1] + cache->roi_miss[cpu][2]; 
  }
  if (TOTAL_ACCESS > 0) {
    cout << cache->NAME;
...
int main(int argc, char** argv)
{
  ...
  static struct option long_options[] = {{"warmup_instructions", required_argument, 0, 'w'},
                                         ...
                                         {"bloom_type", required_argument, 0, 'b'},
                                         {"P", required_argument, 0, 'p'},
                                         {"nHash", required_argument, 0, 'n'},
                                         {"Size", required_argument, 0, 's'},
                                         {"Max", required_argument, 0, 'm'},
                                         {"Pipe", required_argument, 0, 't'},
                                         {"BrainAddress", required_argument, 0, 'a'},
                                         {0, 0, 0, 0}};
  /*
    BloomFilter
  */
  uint16_t nHash = atoi(argv[15]);
  float P = atof(argv[13]);
  uint32_t length = atoi(argv[17]); 
  uint32_t MAX = atoi(argv[19]);
  uint32_t npipe =atoi(argv[21]);
  string bloom_type = argv[11];
  uint32_t BrainAddress = 0; if (nHash==2){ BrainAddress = 32000;}
  

  
  int c;
  
  while ((c = getopt_long_only(argc, argv, "w:i:hc:b:p:s:m:n:f:t:a", long_options, NULL)) != -1 && !traces_encountered) {
    switch (c) {
    case 'w':
  ...
  for (auto it = caches.rbegin(); it != caches.rend(); ++it) {
    ...
    (*it)->impl_replacement_initialize(nHash, P, length, MAX, bloom_type, BrainAddress);
  }
  ...
  for (int i=0;i<4;i++){
    uint64_t TOTAL_ACCESS = BF_TACCESS[i],TOTAL_HIT = BF_THIT[i],TOTAL_MISS = BF_TMISS[i], cycles =ooo_cpu[i]->finish_sim_cycle ;
    //cerr << TOTAL_ACCESS << " " << TOTAL_HIT << " " << TOTAL_MISS << " " << cycles << " " << npipe << endl;
