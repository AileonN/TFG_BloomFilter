/*
------------------------------------------------
    Launch program of champsim simulations: ReD
------------------------------------------------
*/
...
void waitChilds(unsigned int childs_throwed, ofstream* fcsv,unsigned int BANumber, char* SimulationInst){
    uint64_t TOTAL_ACCESS,TOTAL_HIT,TOTAL_MISS, TP,FP,FN,TN, cycles;
    ...
    for (auto i : pipes){
        ...
        read(i.fd_pipe,&TOTAL_ACCESS,sizeof(uint64_t));read(i.fd_pipe,&TOTAL_HIT,sizeof(uint64_t));
        read(i.fd_pipe,&TOTAL_MISS,sizeof(uint64_t));	read(i.fd_pipe,&cycles,sizeof(uint64_t));		
        close(i.fd_pipe);
        if (j==BANumber-1){ //Next trace
          j=0;
          *fcsv << ";" << TOTAL_ACCESS << ";" << TOTAL_MISS << ";" << cycles << ";" << TP << ";" << FP << ";" << FN << ";" << TN << endl;
        }else if (j==0){ //New trace
          j++;
          *fcsv << i.trace << ";" << stoi(SimulationInst) << ";" << TOTAL_ACCESS << ";" << TOTAL_MISS << ";" << cycles << ";" 
          << TP << ";" << FP << ";" << FN << ";" << TN;
        }else{ //Data from trace
          j++;
          *fcsv <<  ";" << TOTAL_ACCESS << ";" << TOTAL_MISS << ";" << cycles << ";" << TP << ";" << FP << ";" << FN << ";" << TN;
        }
...
int main(int argc, char** argv){
    //Check arguments
    if (argc != 7){
        cout << "./init_ReDSimulations warmup_inst simulation_inst file_simulations file_tracelocation file_csv num_childs" << endl;
        return 0;
    }
    ...
    while (!fSim.eof()){
      ...
      fcsv << ";LLC_ACCESS_" << stoi(Simulation) << ";Miss_" << stoi(Simulation) << ";cycles_" << stoi(Simulation) << ";TP_" << stoi(Simulation) << ";FP_" << stoi(Simulation)
	    << ";FN_" << stoi(Simulation) << ";TN_" << stoi(Simulation);
      fSim >> Simulation;
    }
    fcsv << endl;
